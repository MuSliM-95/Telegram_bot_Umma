...
export const message = (bot: Context) => {
    bot.hears('Время молитв', async (ctx: Context) => await ctx.reply('Выберите действие', prayerKeyboardСontainer));
    ...
    bot.on('message', async (ctx: Context) => {
        try {
            ...
            if (!location && idAddress === 'Время') {
                ...
                await prayerTimeCity(ctx, messagePrayer as string[]);
