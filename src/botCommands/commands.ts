...
export const start = async (): Promise<void> => {
  ...
  bot.start((ctx: Context) => {
    ctx.replyWithHTML(
      infoText(),
      ctx?.chat.id == process.env.CHAT_ID! ? adminKeyboard(ctx?.chat.id) : keyboardСontainer(ctx?.chat.id),
    ),
      ctx?.chat.id != process.env.CHAT_ID! &&
  ...
  bot.hears('На главную', (ctx: Context) =>
    ctx.reply(
      'Выберите действие',
      ctx?.chat.id == process.env.CHAT_ID! ? adminKeyboard(ctx?.chat.id) : keyboardСontainer(ctx?.chat.id),
    ),
  ...
  bot.on('message', async (ctx: Context) => {
    try {
      ...
      if (
        (id == process.env.CHAT_ID! && idAddress === 'Umma places') ||
      ...
      if (text === 'Написать администратору') {
        ...
        await ctx.reply(
          `Вам скоро ответят, по воле Аллаха. Также, у нас есть группа, вы можете задать вопрос и там.\nhttps://t.me/+q3g7zPQgT6VmOWRi`,
          closeChat(),
        );
      ...
      if (text === 'Закрыть чат') {
        ...
        await bot.telegram.sendMessage(
          process.env.CHAT_ID!,
          `Пользователь ${first_name} завершил беседу`,
          keyboardСontainer(id),
        );
        return;
      }
